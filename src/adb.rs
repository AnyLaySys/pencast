...
pub(crate) fn provision() -> Result<(), String> {
    ...
    push(&serial, &agent, REMOTE_AGENT)?;
    push(&serial, &worker, "/userdata/.pencast/pencast-work")?;
    adb_shell(
        &serial,
        "chmod 700 /userdata/.pencast/agent.sh /userdata/.pencast/pencast-work\n",
    )?;
    match adb_shell_args(&serial, &["/bin/setsid", "/bin/sh", REMOTE_AGENT, "start"]) {
        Ok(_) => {}
...
fn push(serial: &str, source: &PathBuf, destination: &str) -> Result<(), String> {
    let output = adb()?
        .args(["-s", serial, "push"])
        .arg(source)
        .arg(destination)
...
fn transport_reset(error: &str) -> bool {
    let error = error.to_ascii_lowercase();
    error.contains("closed") || error.contains("not found") || error.contains("offline")
}
